...
export function ForwardMessageDialog({ open, onOpenChange, message }: Props) {
  ...
  async function forward() {
    ...
    try {
      ...
      toast.show({ title: `Forwarded to ${picked.size} chat${picked.size > 1 ? "s" : ""}`, intent: "success" });
      onOpenChange(false);
    } catch (e) {
      ...
      <DialogSurface>
        <DialogBody>
            ...
            <div className={s.wrap}>
              {message ? (
                <div className={s.preview}>
                  {message.body || (message.attachments.length > 0 ? `${message.attachments.length} attachment(s)` : "Message")}
                </div>
              ) : null}
              <Input contentBefore={<SearchRegular />} placeholder="Search conversations" value={q} onChange={(_, d) => setQ(d.value)} />
                  ...
                  return (
                    <label key={c.id} className={s.row}>
                      ...
                      <div className={s.info}>
                        <Text weight="semibold" truncate wrap={false}>{conversationTitle(c)}</Text>
                        <Text size={200} style={{ color: tokens.colorNeutralForeground3 }} truncate wrap={false}>
                          {c.kind === "group" ? `${c.memberIds.length} members` : "Direct message"}
                        </Text>
                      </div>
                    </label>
                  );
          ...
          <DialogActions>
            ...
            <Button appearance="primary" icon={<ShareRegular />} onClick={forward} disabled={picked.size === 0 || sending}>
